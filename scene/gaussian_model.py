...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.denom = torch.empty(0)   # denominator 分母，grads = self.xyz_gradient_accum / self.denom
                                      # 代表每个高斯被更新的次数，从而得到累计平均梯度
                                      # 所谓累计，指的是每次剪枝克隆的之间（因为 densification_postfix 造新高斯的时候会清空），符合直觉

        self.optimizer = None  #初始化优化器为 None。
        self.percent_dense = 0  #初始化百分比密度为0。用于得到阈值，决定 split/clone
    ...
    def capture(self):
        return (
            ...
            self._opacity,
            self.max_radii2D,
            self.xyz_gradient_accum,  # 梯度累积的值，用于优化过程（如剪枝）
            self.denom,               # denominator 分母，grads = self.xyz_gradient_accum / self.denom
                                      # 代表每个高斯被更新的次数，从而得到累计平均梯度
                                      # 所谓累计，指的是每次剪枝克隆的之间（因为 densification_postfix 造新高斯的时候会清空），符合直觉
    ...
    def replace_tensor_to_optimizer(self, tensor, name): # 重置3DGS高斯点的opacity优化器参数状态，重新开始学习opacity
        optimizable_tensors = {}
        for group in self.optimizer.param_groups: # self.optimizer.param_groups返回的是列表，即torch.optim.Adam(l, lr=0.0, eps=1e-15)中的 l,列表中的每个元素都是字典
            if group["name"] == name: # group是字典，{'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"}
                """
                group['params'][0]是self._opacity，优化器state是双层字典。
                第一层中key为每个高斯点的opacity参数，values为字典，包含优化器参数（adam包括step、exp_avg、exp_avg_sq）
                optimizer.state 是这样组织的：
                {
                    param_tensor_1: { ... 状态 ... },
                    param_tensor_2: { ... 状态 ... },
                    ...
                ...
                """
                stored_state = self.optimizer.state.get(group['params'][0], None) # stored_state为self._opacity中在优化器的参数（adam包括step、exp_avg、exp_avg_sq）
                stored_state["exp_avg"] = torch.zeros_like(tensor)
                stored_state["exp_avg_sq"] = torch.zeros_like(tensor)

                del self.optimizer.state[group['params'][0]] # tensor 在 Python 中可用作 dict 的 key（基于对象 id）
    ...
    def _prune_optimizer(self, mask): # 将需要删掉的高斯点从优化器中删掉，即需要修建掉的高斯点的优化器参数删掉
        optimizable_tensors = {}
    ...
    def cat_tensors_to_optimizer(self, tensors_dict): # 将稠密化增加的高斯点放进优化器中
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # 一步过滤掉那些缩放（scaling）小于 一定百分比的场景范围的点。滤掉过小的高斯点
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if max_screen_size: #设置了高斯最大半径的阈值，
            big_points_vs = self.max_radii2D > max_screen_size #创建一个掩码，标记在图像空间中半径大于指定阈值的点。
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent #创建一个掩码，标记在世界空间中尺寸大于指定阈值（camera_extent）的点。
